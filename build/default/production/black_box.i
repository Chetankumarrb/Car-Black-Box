...
void Dashboard_diplay() {
    ...
    clcd_print("E", (0x80 + (11)));
    clcd_print("SP", (0x80 + (14)));

    clcd_print(arr[index], (0xC0 + (11)));
    display_time();

    if (index == 8 && key == 1)
        index = 1;
    else if (index < 7 && key == 1)
        index++;
    else if (key == 2 && index > 1 && index < 8)
        index--;
    else if (key == 3)
        index = 8;
    ...
    if (secC >= 3 && key == 0xFF) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            if (orgArr[j] != temp[j] && c != 1) {
                ...
                clcd_putch((sec / 10) % 10 + '0', (0xC0 + (10)));
                clcd_putch(sec / 100 + '0', (0xC0 + (9)));
                if (sec == 0) {
                    clcd_write(0x01, 0);
                    func_flag = 0;
                    sec = 120;
...
void main_menu() {
    ...
    if(key == 12 && key_press == 2)
    {
        clcd_write(0x01, 0);
        func_flag = 0;
        key_press = 0;
    }
    else if (key == 11 && key_press == 2)
    {
        func_flag = 3 + m_index;
        clcd_write(0x01, 0);
        key_press = 0;
    ...
    else
    {
        ...
        {
            clcd_print("*", (0x80 + (0)));
            clcd_print(menu[m_index], (0x80 + (2)));
            if (m_index != 4)
            {
                clcd_print(menu[m_index + 1], (0xC0 + (2)));
            }
        }
        if (m_index % 2 == 1)
        {
            clcd_print("*", (0xC0 + (0)));
            clcd_print(menu[m_index - 1], (0x80 + (2)));
            clcd_print(menu[m_index], (0xC0 + (2)));
        }
    }
}
...
void View_Log() {
    if (Clear_log_flag == 0) {
        ...
        if (view_flag == 0) {
            for (int i = 0; i < 12; i++) {
                clcd_putch(read_external_eeprom(add++), (0xC0 + (i)));
            }
            view_flag = 1;
        }
        if (view_flag) {
...
void Set_time() {
    ...
    clcd_write(0x01, 0);
    get_time();
    set_dummy = read_ds1307(0x00);
    write_ds1307(0x00, set_dummy | 0x80);
    ...
    while (1) {
        if (wait++ <= 1000) {
            clcd_putch((seconds / 10) + '0', (0xC0 + (8)));
            clcd_putch((seconds % 10) + '0', (0xC0 + (9)));
            clcd_putch((minutes / 10) + '0', (0xC0 + (5)));
            clcd_putch((minutes % 10) + '0', (0xC0 + (6)));
            clcd_putch((hours / 10) + '0', (0xC0 + (2)));
            clcd_putch((hours % 10) + '0', (0xC0 + (3)));
        }
        ...
        if (s_key == 11 && key_press == 2)
        {
            key_press = 0;

            write_ds1307(0x00, (seconds % 10) | ((seconds / 10) << 4));
            write_ds1307(0x01, (minutes % 10) | ((minutes / 10) << 4));
            write_ds1307(0x02, (hours % 10) | ((hours / 10) << 4));
            set_dummy = read_ds1307(0x00);

            write_ds1307(0x00, set_dummy & 0x7F);
            clcd_write(0x01, 0);
            clcd_print("TIME SET SUCC", (0x80 + (0)));
            _delay((unsigned long)((2100)*(20000000/4000.0)));
            func_flag = 2;
            clcd_write(0x01, 0);
            return;
        }
        else if(s_key == 12 && key_press == 2)
        {
            key_press = 0;
            func_flag =2;
            return;
